			...
			synchronized (this) {
				...
				if (sqlUnit == null) { // double-check
					...
					Table meta = TableMetaGenerator.getTableMeta(context.getTargetDs(),
							context.isIgnoreSchema() ? null : names.get(0), names.get(1));
					...
					if (useMerge && YuGongUtils.isNotEmpty(meta.getColumns())) {
						...
						if (dbType == DbType.MYSQL) {
							applierSql = SqlTemplates.MYSQL.getMergeSql(meta.getSchema(), meta.getName(), primaryKeys,
									columns, true);
						} else if (dbType == DbType.DRDS) {
							applierSql = SqlTemplates.MYSQL.getMergeSql(meta.getSchema(), meta.getName(), primaryKeys,
									columns, false);
						} else if (dbType == DbType.ORACLE) {
							applierSql = SqlTemplates.ORACLE.getMergeSql(meta.getSchema(), meta.getName(), primaryKeys,
									columns);
						}
					} else {
						if (YuGongUtils.isEmpty(meta.getColumns()) && dbType == DbType.MYSQL) {
							// 如果mysql，全主键时使用insert ignore
							applierSql = SqlTemplates.MYSQL.getInsertSql(meta.getSchema(), meta.getName(), primaryKeys,
									columns);
						} else {
							applierSql = SqlTemplates.COMMON.getInsertSql(meta.getSchema(), meta.getName(), primaryKeys,
									columns);
						}
					}
			...
			synchronized (this) {
				...
				if (sqlUnit == null) { // double-check
					...
					if (useMerge && YuGongUtils.isNotEmpty(meta.getColumns())) {
						...
						applierSql = SqlTemplates.COMMON.getUpdateSql(meta.getSchema(), meta.getName(), primaryKeys,
								columns);
					}
