...
public class RecordDumper {
	...
	public static void dumpExtractorInfo(Long batchId, List<Record> records, Position lastPosition,
			boolean dumpDetail) {
		extractorLogger.info(SEP + "****************************************************" + SEP);
		Date now = new Date();
		SimpleDateFormat format = new SimpleDateFormat(TIMESTAMP_FORMAT);
		extractorLogger.info(MessageFormat.format(extractor_format, String.valueOf(batchId), records.size(),
				ObjectUtils.toString(lastPosition), format.format(now)));
	...
	public static void dumpApplierInfo(Long batchId, List<Record> extractorRecords, List<Record> applierRecords,
		...
		Date now = new Date();
		SimpleDateFormat format = new SimpleDateFormat(TIMESTAMP_FORMAT);
		applierLogger.info(MessageFormat.format(applier_format, String.valueOf(batchId), extractorRecords.size(),
				applierRecords.size(), ObjectUtils.toString(position), format.format(now)));
	...
	public static String dumpRecord(Record record) {
		...
		return MessageFormat.format(record_format, record.getSchemaName(), record.getTableName(), type,
				dumpRecordColumns(record.getPrimaryKeys()), dumpRecordColumns(record.getColumns()));
	}
