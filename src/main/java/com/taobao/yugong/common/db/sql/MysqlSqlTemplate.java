...
public class MysqlSqlTemplate extends SqlTemplate {
	...
	public String getMergeSql(String schemaName, String tableName, String[] pkNames, String[] colNames,
		...
		if (mergeUpdatePk) {
			for (int i = 0; i < size; i++) {
				sql.append(getColumnName(allColumns[i])).append("=values(").append(getColumnName(allColumns[i]))
						.append(")");
			...
			for (int i = 0; i < colNames.length; i++) {
				sql.append(getColumnName(colNames[i])).append("=values(").append(getColumnName(colNames[i]))
						.append(")");
