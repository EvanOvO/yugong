...
public class YuGongController extends AbstractYuGongLifeCycle {
	...
	public void start() {
		...
		Collection<TableHolder> tableMetas = initTables();// 表信息
		...
		String alarmReceiver = config.getString("yugong.alarm.receiver", "");// 邮箱地址
	...
	private RecordExtractor chooseExtractor(TableHolder tableHolder, YuGongContext context, RunMode runMode,
		...
		if (runMode == RunMode.FULL || runMode == RunMode.CHECK) {// 全量模式或对比模式
		...
		} else if (runMode == RunMode.INC) {// 增量模式
			//
			if (sourceDbType == DbType.ORACLE) {
			...
			//
		} else if (runMode == RunMode.MARK || runMode == RunMode.CLEAR) {// 记录模式或清除模式
			if (sourceDbType == DbType.ORACLE) {
			...
			AbstractRecordExtractor markExtractor = (AbstractRecordExtractor) chooseExtractor(tableHolder, context,
					RunMode.MARK, positioner);
			AbstractRecordExtractor fullExtractor = (AbstractRecordExtractor) chooseExtractor(tableHolder, context,
					RunMode.FULL, positioner);
			AbstractRecordExtractor incExtractor = (AbstractRecordExtractor) chooseExtractor(tableHolder, context,
					RunMode.INC, positioner);
	...
	private Collection<TableHolder> initTables() {
		...
		DbType targetDbType = YuGongUtils.judgeDbType(globalContext.getTargetDs());// 目标数据库类型
		if (!isEmpty) {
			for (Object obj : tableWhiteList) {
				...
				if (!isBlackTable(whiteTable, tableBlackList)) {
					...
					if (strs.length == 1) {
						whiteTables = TableMetaGenerator.getTableMetasWithoutColumn(globalContext.getSourceDs(), null,
								strs[0]);
						ignoreSchema = true;
	...
	private DataTranslator buildExtKeys(Table table, String tableStr, DbType targetDbType) {
		...
		if (targetDbType.isDRDS()) {
			...
			String drdsExtKey = TableMetaGenerator.getShardKeyByDRDS(globalContext.getTargetDs(), schemaName,
					tableName);
