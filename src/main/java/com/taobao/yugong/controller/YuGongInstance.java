					...
					try {
						...
						if (exception == null) {
							...
							logger.info("table[{}] is interrpt ,current status:{} !",
									context.getTableMeta().getFullName(), extractor.status());
						} else {
							progressTracer.update(context.getTableMeta().getFullName(), ProgressStatus.FAILED);
...
//								System.out.println(sb.toString());
							...
							// 判断是否记录日志
							RecordDumper.dumpExtractorInfo(batchId.incrementAndGet(), ackRecords, lastPosition,
									extractorDump);
				...
				private boolean processException(Throwable e, int i) {
					...
					if (!(ExceptionUtils.getRootCause(e) instanceof InterruptedException)) {
						...
						try {
							alarmService
									.sendAlarm(new AlarmMessage(ExceptionUtils.getFullStackTrace(e), alarmReceiver));
						} catch (Throwable e1) {
